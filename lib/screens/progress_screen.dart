...
class _ProgressScreenState extends State<ProgressScreen> {
      ...
      builder: (context, snapshot) {
        ...
        return ListView(
          ...
          children: <Widget>[
            ...
            const SizedBox(height: 16),
            switch (_view) {
              _ProgressView.calendar => _CalendarView(
                  records: records,
              ...
              _ProgressView.week => _WeekView(records: records),
              _ProgressView.streak => _StreakView(records: records),
            },
          ],
        );
      },
  ...
  String _subtitleForView(_ProgressView view) {
    return switch (view) {
      _ProgressView.calendar => 'Your routine history, day by day.',
      _ProgressView.week => '14 sessions. One clear weekly view.',
      _ProgressView.streak => 'Build consistency one full day at a time.',
    };
  }
}
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              const Padding(
                ...
                child: Row(
                  children: <Widget>[
                    for (final label in <String>['S', 'M', 'T', 'W', 'T', 'F', 'S'])
                      Expanded(
                        child: Center(
                          child: Text(
                            label,
...
class _CalendarDay extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (selected && state != DayCompletionState.complete) {
      border = SkinFlowColors.primary;
    }
          ...
          children: <Widget>[
            ...
            ClipRRect(
              ...
              child: LinearProgressIndicator(
                value: (completed / 14).clamp(0.0, 1.0),
                minHeight: 8,
              ),
            ),
          ],
...
class _StreakView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final progress = metrics.nextMilestone == 0
        ? 0.0
        : (metrics.current / metrics.nextMilestone).clamp(0.0, 1.0);
    ...
    return Column(
      children: <Widget>[
        ...
        const ListTile(
          ...
          leading: Icon(Icons.stars_outlined, color: SkinFlowColors.primaryText),
          title: Text(
...
String _stateLabel(DayCompletionState state) => switch (state) {
      DayCompletionState.complete => 'Complete',
      DayCompletionState.partial => 'Partial',
      DayCompletionState.missed => 'Missed',
      DayCompletionState.pending => 'Pending',
    };
...
String _longDate(DateTime date) =>
    '${const <String>['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'][date.weekday % 7]}, ${const <String>['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'][date.month - 1]} ${date.day}';
